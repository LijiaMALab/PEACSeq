...
def get_sample_id(i1, i2, sample_names):
    seq1 = i1[1]
    seq2 = i2[1]
    sample_barcode = (seq1[1:8] + seq2[1:8]).decode()
...
def demultiplex(read1, read2, index1, index2, sample_barcodes, out_dir, min_reads=10000):
    ...
    if not isinstance(sample_barcodes, dict):
        sample_names = {}
    ...
    start = time.time()
    # byte2str = list(map(lambda x:x.decode() if isinstance(x,bytes) else str(x), record))
    def byte2str(alist):
        if isinstance(alist, list):
            return list(map(lambda x:x.decode() if isinstance(x,bytes) else str(x), alist))
        return alist.decode() if isinstance(alist, bytes) else str(alist)
    ...
    for r1,r2,i1,i2 in zip(fq(read1), fq(read2), fq(index1), fq(index2)):
        ...
        if count[sample_id] < min_reads:
            ...
            for record in buffer_r1[sample_id] + r1:
                outfiles_r1[sample_id].write(''.join(byte2str(record)))
            for record in buffer_r2[sample_id] + r2:
                outfiles_r2[sample_id].write(''.join(byte2str(record)))
            for record in buffer_i1[sample_id] + i1:
                outfiles_i1[sample_id].write(''.join(byte2str(record)))
            for record in buffer_i2[sample_id] + i2:
                outfiles_i2[sample_id].write(''.join(byte2str(record)))
            ...
            for line in r1:
                print (byte2str(line), file=outfiles_r1[sample_id], end="")
            for line in r2:
                print (byte2str(line), file=outfiles_r2[sample_id], end="")
            for line in i1:
                print (byte2str(line), file=outfiles_i1[sample_id], end="")
            for line in i2:
                print (byte2str(line), file=outfiles_i2[sample_id], end="")
    ...
    for sample_id in buffer_r1.keys():
        for record in buffer_r1[sample_id]:
            undetermined_r1.write(''.join(byte2str(record)))
        for record in buffer_r2[sample_id]:
            undetermined_r2.write(''.join(byte2str(record)))
        for record in buffer_i1[sample_id]:
            undetermined_i1.write(''.join(byte2str(record)))
        for record in buffer_i2[sample_id]:
            undetermined_i2.write(''.join(byte2str(record)))
